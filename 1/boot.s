...
search for this signature in the first 8KiB of the kernel file, aligned at a 32-bit
...
the kernel file is smaller because it does not contain a uninitialized stack.
...
and the failure to align the stack will result in undefined behavior.
*/
.section .bss
.align 16
stack_bottom:
...
_start:
	...
	*/
	...
	/*
	Call _init to initialize Global Constructors from the crt*.o files.
	*/
	call _init

    /*
	Enter the high-level kernel. The ABI requires the stack is 16-byte aligned at the
	time of the call instruction (which afterwards pushes the return pointer of size
    4 bytes). The stack was originally 16-byte aligned above and we've pushed a multiple
    of 16 bytes to the stack since (pushed 0 bytes so far), so the alignment has thus been
